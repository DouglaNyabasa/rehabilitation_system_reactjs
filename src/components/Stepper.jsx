  ...
  const displaySteps = newStep.map((step, i) => {
      return (
          <div key={i} className={i !== newStep.length - 1 ? ` w-full flex items-center` : `flex items-center`}>
              ...
              <div className='relative flex flex-col items-center text-primaryColor'>
                  <div className={`rounded-full transition duration-500 ease-in-out border-2 border-gray-300 h-12 w-12 flex items-center justify-center py-3 ${step.selected ? 'bg-blue-800 text-white font-bold border border-primaryColor' : ''}`}> {/* display number */}{step.completed ? (
